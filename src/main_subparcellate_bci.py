...
#        bci_labs[(bci_bst.labels == roino*10) & (bci_labs > 0) ] = 1 
#        bci_labs[(bci_bst.labels == roino*10) & (bci_labs == 0) ] = 2#sp.amax(bci_labs) + 1
#        bci_labs[freq1 == 0] = 0
    ...
    freq1[ (bci_labs > 0) & (freq1 == 0)] = sp.amin(freq1[freq1 != 0])
    print sp.amax(bci_labs)
...
bci_bst = patch_color_labels(bci_bst, freq=freq, cmap='jet')
...
bci_bst = patch_color_attrib(bci_bst, sp.double(sp.mod(bci_bst.labels,10)==0))
view_patch_vtk(bci_bst, show=1)
