...
for ind1 in range(1, nSub):
    for ind2 in range(1, nSub):
        ...
        sub_data_rot, _ = rot_sub_data(ref=sub_data[:, :, ind1],
                                       sub=sub_data[:, :, ind2])
        dist_all_rot += sp.mean((sub_data[:, :, ind1] - sub_data_rot)**2.0,
                                axis=(1))
        print ind1, ind2

dist_all_rot = dist_all_rot/(nSub*nSub)
dist_all_orig = dist_all_orig/(nSub*nSub)
...
dfs_right_sm = patch_color_attrib(dfs_right_sm, dist_all_orig, clim=[0, 1])
view_patch_vtk(dfs_right_sm, azimuth=-90, elevation=-180,
               roll=-90, outfile='dist_orig_view1_1sub_right.png', show=0)
view_patch_vtk(dfs_right_sm, azimuth=90, elevation=180, roll=90,
               outfile='dist_orig_view2_1sub_right.png', show=0)

dfs_right_sm = patch_color_attrib(dfs_right_sm, dist_all_rot, clim=[0, 1])
view_patch_vtk(dfs_right_sm, azimuth=-90, elevation=-180, roll=-90,
               outfile='dist_rot_view1_1sub_right.png', show=0)
view_patch_vtk(dfs_right_sm, azimuth=90, elevation=180, roll=90,
               outfile='dist_rot_view2_1sub_right.png', show=0)

dfs_right_sm = patch_color_attrib(dfs_right_sm,
                                  (2.0 - dist_all_orig)/2.0, clim=[0, 1])
view_patch_vtk(dfs_right_sm, azimuth=-90, elevation=-180,
...
dfs_right_sm = patch_color_attrib(dfs_right_sm,
                                  (2.0 - dist_all_rot)/2.0, clim=[0.5, 1])
...
sp.savez('rot_pairwise_dist_all.npz', dist_all_rot)
