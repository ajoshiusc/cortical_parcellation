...
#        B[(jj*nNodes+np.arange(nNodes)),(kk*nNodes+np.arange(nNodes))]=1
...
for nClusters in [6]:
    ...
    for kk in range(nSub):
        r = dfs_left_sm
        r.labels = r.labels*0
        r.labels[msk_small_region] = labs[kk*nNodes+np.arange(nNodes)]+1
        mesh = mlab.triangular_mesh(r.vertices[:, 0], r.vertices[:, 1],
        ...
        mlab.savefig(filename = 'c'+str(nClusters)+'s'+str(kk)+'labels1_2.png')
 #       mlab.view(azimuth=0, elevation=90)
